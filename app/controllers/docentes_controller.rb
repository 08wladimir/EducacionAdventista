class DocentesController < ApplicationController
  ...
  def index
    ...
    if current_usuario.rol == "Administrador Colegio"
      @docentes = Docente.where("creado_by = ?", current_usuario.usuario)
    end
  end
  ...
  private
    ...
    def docente_params
      params.require(:docente).permit(:cedula, :nombre, :apellido, :telefono, :email, :area, :titulos, :cargo, :colegio_id)
    end
end
